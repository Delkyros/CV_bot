...
| `LOCATION_BLOCKLIST_MIN_ERRORS` | `2` | How many `Localidade/Modelo incorreto` marks a company needs before it is skipped on sight (`main.learned_location_blocklist`). A company with any `viewed`/`applied` job is never blocked. Do not set to `1`: measured on the history, 80% of the offending companies produced exactly one error, so `1` adds ~112 companies of blast radius for errors it could not have predicted anyway. |
